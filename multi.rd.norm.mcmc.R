multi.rd.norm.mcmc <- function(y,X,player,n.mcmc){
...
loglik <- function(y,mu,sig,theta,w) { # vectorized for d and mu
	thresh=.000000001
  y=as.vector(y)
	mu=as.vector(mu)
	Y.1=outer(y,(theta+1)/2,"+")
	Y.2=outer(y,(theta-1)/2,"+")
	dens=(pnorm(Y.1,mu,sig)-pnorm(Y.2,mu,sig))%*%w/2
	dens[dens<thresh]=thresh
	sum(log(dens))
}

loglik.nosum <- function(y,mu,sig,theta,w) { # vectorized for d and mu
	thresh=.000000001
  y=as.vector(y)
	mu=as.vector(mu)
	Y.1=outer(y,(theta+1)/2,"+")
...
intloglik <- function(y,mu0,sig0,mu,sig,theta,w,z.gh,w.gh,grp){  # G-H Quad 
  n=max(grp)
  tmp.sum=0
  for(i in 1:n){
    beta0.mat=matrix(mu0+sqrt(2)*sig0*z.gh,length(y[grp==i]),length(z.gh),byrow=TRUE)
    ll.vec=apply(matrix(loglik.nosum(rep(y[grp==i],length(z.gh)),c(beta0.mat+mu[grp==i]),sig,theta,w),length(y[grp==i]),length(z.gh)),2,sum)
    ll.max=max(ll.vec)
...
for(k in 1:n.mcmc){
  ...
  sig.star=sqrt(s2.star)
    mh.1=loglik(y,beta.0.big+Xbeta,sig.star,theta,w)+ldIG(s2.star,q,r)
    mh.2=loglik(y,beta.0.big+Xbeta,sig,theta,w)+ldIG(s2,q,r)
    mh=exp(mh.1-mh.2)
    if(mh>runif(1)){
      s2=s2.star	
      sig=sig.star
  ...
  mh.1=loglik(y,beta.0.big+Xbeta.star,sig,theta,w)+sum(dnorm(beta.star,mu.beta,sig.beta,log=TRUE))
  mh.2=loglik(y,beta.0.big+Xbeta,sig,theta,w)+sum(dnorm(beta,mu.beta,sig.beta,log=TRUE))
  mh=exp(mh.1-mh.2)
  if(mh>runif(1)){
    beta=beta.star	
    Xbeta=Xbeta.star
  ...
  for(i in 1:n){
    ...
    mh.1=loglik(y[player==i],beta.0.star+Xbeta[player==i],sig,theta,w)+dnorm(beta.0.star,mu.0,sig.0,log=TRUE)
    mh.2=loglik(y[player==i],beta.0[i]+Xbeta[player==i],sig,theta,w)+dnorm(beta.0[i],mu.0,sig.0,log=TRUE)
    mh=exp(mh.1-mh.2)
    if(mh>runif(1)){
  ...
  Dbar.save[k]=-2*intloglik(y,mu.0,sig.0,Xbeta,sig,theta,w,z.gh,w.gh,player)
...
Dhat=-2*intloglik(y,mu.0.mn,sig.0.mn,X%*%beta.mn,sig.mn,theta,w,z.gh,w.gh,player)
pD=Dbar-Dhat
DIC=Dhat+2*pD
...
list(y=y,X=X,n.mcmc=n.mcmc,beta.save=beta.save,sig.save=sig.save,beta.0.save=beta.0.save,mu.0.save=mu.0.save,s2.0.save=s2.0.save,DIC=DIC,pD=pD,Dbar=Dbar,Dhat=Dhat)	
